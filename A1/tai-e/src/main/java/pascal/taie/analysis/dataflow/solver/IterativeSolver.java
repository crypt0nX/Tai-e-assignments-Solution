        ...
        while(INChanged) {
            ...
            for (Node node : cfg) {
                ...
                for (Node successor : cfg.getSuccsOf(node)) {
                    out.union((SetFact<Var>) result.getInFact(successor)); // OUT[B] = U S a successor of B IN[S];
                }
                if (analysis.transferNode(node, (Fact) out, (Fact) in)){ // IN[B] = use B U (OUT[B] - defB;
                    INChanged = true;
                }
